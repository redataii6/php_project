...
<body>
    ...
    <div class="w-[90%] mx-auto md:flex justify-between ">
        <form method="GET" action="{{ route('produits.categorie') }}" class="hidden md:block">
            <div class="sticky bg-white h-fit w-[250px] top-[100px] rounded-[20px]">
                <div class="flex">
                    <img src="{{ asset('filtre.png') }}" class="w-5 h-5 mt-5 ml-5">
                    <h1 class="mt-3.5 ml-2 font-bold text-2xl">Filtre</h1>
                    <a href="{{ route('produits.categorie', ['id' => $categorie ? $categorie->id : null]) }}">
                        <img class="w-7 h-7 mt-3.5 ml-25" src="{{ asset('circulaire.png') }}" alt="reinitialiser">
                    </a>
                </div>

                <!-- Taille -->
                <div class="ml-2">
                    <h1 class="mt-5 ml-5 font-medium text-xl">Taille</h1>
                    <div class="grid grid-cols-2 mt-2 ml-7">
                        @foreach($tailles as $taille)
                        <div class="flex items-center">
                            <input type="checkbox" name="f[taille][]" value="{{ $taille->id }}" {{ in_array($taille->id, request()->input('f.taille', [])) ? 'checked' : '' }}>
                            <label class="ml-2">{{ $taille->size }}</label>
                        </div>
                        @endforeach
                    </div>
                </div>
                ...
                <!-- Couleur -->
                <div class="ml-2">
                    <h1 class="mt-5 ml-5 font-medium text-xl">Couleur</h1>
                    <div class="grid grid-cols-2 mt-2 ml-7">
                        @foreach($couleurs as $couleur)
                        <div class="flex items-center">
                            <input type="checkbox" name="f[couleur][]" value="{{ $couleur->id }}"  {{ in_array($couleur->id, request()->input('f.couleur', [])) ? 'checked' : '' }}>
                            <label class="ml-2">{{ $couleur->name }}</label>
                        </div>
                        @endforeach
                    </div>
                </div>
                ...
                <!-- Prix min -->
                <div class="ml-2">
                    <h1 class="mt-5 ml-5 font-medium text-xl">Prix min</h1>
                    <div class=" mt-2 ml-7">
                        <input type="number" name="f[prix_min]" class="border-b-2 border-black w-[100px] h-[30px]" maxlength="5" value="{{ request('f.prix_min') }}">
                    </div>
                </div>

                <!-- Prix max -->
                <div class="ml-2">
                    <h1 class="mt-5 ml-5 font-medium text-xl">Prix max</h1>
                    <div class=" mt-2 ml-7">
                        <input type="number" name="f[prix_max]" class="border-b-2 border-black w-[100px] h-[30px]" maxlength="5" value="{{ request('f.prix_max') }}">
                    </div>
                </div>

                <!-- Appliquer -->
                <div class="flex justify-center mt-6 py-4 ml-30">
                    <button type="submit" class="bg-black text-white font-bold py-2 px-1 rounded-[10px] w-[100px] h-[40px]">Appliquer</button>
                </div>
            </div>
        </form>
        <div class="grid grid-cols-2 md:grid-cols-4 md:gap-1 md:w-[82%] w-[100%] bg-white rounded-[20px]">
            @foreach ($products as $product)

            <div class="bg-white rounded-lg p-1.5 md:p-4 h-auto">
                <a href="{{ route('produit.details', ['id' => $product->id]) }}"><img src="{{ asset('storage/' . $product->firstPhoto->image_path) }}" alt="" class="w-[269px] h-[403px] object-cover rounded-md"></a>
                <div class="flex">
                    <a href="{{ route('produit.details', ['id' => $product->id]) }}">
                        <h2 class="mt-2">{{ $product->nom }}</h2>
                    </a>
                    <img id="favori-{{ $product->id }}" class="self-start mt-2 mr-2 ml-auto w-[24px] " src="{{ auth()->user() && auth()->user()->favorites->contains($product->id) ? asset('favori.png') : asset('favoris.png') }}" alt="favoris" onclick="toggleFavori({{ $product->id }})">
                </div>
                <a href="{{ route('produit.details', ['id' => $product->id]) }}">
                    <div class="flex">
                        @for ($i = 1; $i <= 5; $i++)
                    ...
                    <p class="text-lg font-semibold ">{{ $product->prix }} MAD</p>
                </a>
            </div>
