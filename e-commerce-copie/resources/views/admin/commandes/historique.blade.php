        ...
        <div class="ml-70 w-[80%]">
            ...
            <div class="mt-17">
                <div class="flex justify-between items-center mb-5">
                    <h1 class=" font-extrabold text-5xl mb-5 ml-5" style="font-family: 'Apple Chancery';">Historique Des Commandes</h1>
                </div>
                <p class="ml-16">Utilisez le filtre pour effectuer une recherche plus rapide.</p>
                <form method="get" action="{{ route('admin.commandes.historique') }}" class="mt-8">
                    @csrf
                    ...
                    <div class="flex justify-end mb-16 mt-8 mr-22">
                        <a href="{{ route('admin.commandes.historique') }}" class="mr-5 mt-2 font-medium">Réinitialiser</a>
                        <button type="submit"
            ...
            <form method="GET" action="{{ route('admin.commandes.historique') }}" class="mb-4">
                <select name="mois" id="mois" onchange="this.form.submit()" class="block w-40 rounded-md px-3.5 py-[11.5px] outline-2  outline-black">
                    @foreach ($moisDisponibles as $moisKey => $commandesGroup)
                    @php
                    $moisLabel = \Carbon\Carbon::createFromFormat('Y-m', $moisKey)->translatedFormat('F Y');
                    @endphp
                    <option value="{{ $moisKey }}" {{ $moisSelectionne === $moisKey ? 'selected' : '' }}>
                        {{ $moisLabel }}
                    </option>
                    @endforeach
                </select>
