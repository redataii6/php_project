...
<body>
    ...
    <div class="flex w-[90%] mx-auto mt-10 mb-10">
        <div class="w-[20%] bg-white rounded-[20px] p-4 h-[300px]">
            <div class="flex">
                <img class="w-10 h-10" src="https://dfcdn.defacto.com.tr/AssetsV2/dist/img/account/welcome.svg" alt="">
                <h1 class="font-bold mt-2 ml-2">{{ Auth::user()->name }}</h1>
            </div>
            <div class="p-6">
                <a href="{{ route('mon.compte') }}">
                    <p class="underline decoration-2 font-semibold text-lg">Mon Compte</p>
                </a>

                <a href="{{ route('commandes.index') }}">
                    <p class="font-semibold text-lg text-gray-600 mt-2">Mes Commandes</p>
                </a>
            </div>

        </div>

        <div class="bg-white rounded-[20px] p-8 ml-10">
            <form action="{{ route('mon.compte') }}" method="POST">
                ...
                <div class="flex justify-between w-[620px]">
                    <div class="flex inputGroup">
                        <input class="!w-[300px]" type="text" name="name" value="{{ old('name', Auth::user()->name) }}" required>
                        <label for="nom" class="!bg-white mt-[1px]">Nom</label>
                    </div>
                    <div class="flex inputGroup">
                        <input class="!w-[300px]" type="email" name="email" value="{{ old('email', Auth::user()->email) }}" required>
                        <label for="email" class="!bg-white mt-[1px]">Email</label>
                    </div>
                </div>

                <hr class="mt-5 mb-5">

                <h2 class="text-2xl font-semibold mb-8">Modifier mot de passe</h2>
                <div class="flex inputGroup">
                    <input class="!w-[300px]" type="password" name="current_password">
                    <label for="password" class="!bg-white mt-[1px]">Mot de passe actuel</label>
                </div>

                <div class="flex justify-between">
                    <div class="flex inputGroup">
                        <input class="!w-[300px]" type="password" name="new_password">
                        <label for="new_password" class="!bg-white mt-[1px]">Nouveau mot de passe</label>
                    </div>
                    <div class="flex inputGroup">
                        <input class="!w-[300px]" type="password" name="new_password_confirmation">
                        <label for="new_password_confirmation" class="!bg-white mt-[1px]">Confirmation</label>
                    </div>
                </div>

                <button type="submit" class="float-end py-2 px-3 font-semibold text-white text-center border bg-black rounded-[10px]">Mettre à jour</button>
            </form>
        </div>
    </div>
